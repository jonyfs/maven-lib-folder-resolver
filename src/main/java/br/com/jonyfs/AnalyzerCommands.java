...
@Slf4j
@ShellComponent
public class AnalyzerCommands {

    @ShellMethod("Analyze directory. Example: analyze path-to-jars-folder")
    public void analyze(@ShellOption() @NotEmpty String directory) throws IOException {
        analyzeWithRepo(directory, directory);
    }

    @ShellMethod("Analyze directory with specific repository. Example: analyze path-to-jars-folder path-to-repository")
    public void analyzeWithRepo(@ShellOption() @NotEmpty String directory, @ShellOption() @NotEmpty String repository) throws IOException {
        ...
        if (Files.isDirectory(Paths.get(directory))) {
            ...
            Files.newDirectoryStream(Paths.get(directory),
                ...
                .forEach((jar) -> analyze(jar, repository));
        } else {
            LOGGER.error("{} is not a directory", directory);
        }
    }

    private void analyze(Path file, String repository) {
        try {
